    ...
    const fetchBrand = async () => {
        ...
        try {
            const res = await fetch("/relayapi", {
                ...
                headers: {
                    ...authHeaders(),
                    endpoint: `brand-details/${id}`,
                    module: "brand",
                },
            });
            const payload = await res.json();
            const data = payload.encrypted ? decryptResponse(payload.encrypted) : payload;
            setBrand(data?.brandId ? data : null);
        } catch (err) {
            console.error(err);
            setBrand(null);
        } finally {
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-[#f5f6f8]">
                <Header page="brand-details" />
                <div className="flex items-center justify-center py-20">
                    <Loader label="Loading brand details..." />
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-[#f5f6f8] text-gray-800">
            ...
            <div className="w-full px-4 sm:px-6 lg:px-8 py-4 pb-20">
                ...
                <nav className="mb-6 flex items-center space-x-2 text-sm font-medium text-gray-500">
                    ...
                    <span
                        className="cursor-pointer hover:text-blue-600 hover:underline"
                        onClick={(e) => gotoPages(e, "/brand-list")}
                    >
                        Brands
                    </span>
                    <span className="text-gray-400">{">>"}</span>
                            ...
                            <div className="rounded-2xl bg-white p-6 shadow-sm">
                                <div className="flex items-center gap-4 border-b pb-5">
                                    <div className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-full bg-blue-50 shadow-md">
                                        <span className="text-2xl font-bold text-blue-600 uppercase">
                                            {brand.brandCode?.substring(0, 2) || "BR"}
                                        </span>
                                    </div>
                                    <div>
                                ...
                                <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-[#101010] font-bold text-[#374151] mt-1">
                                            {brand.manufacturerName || brand.manufacturer?.manufacturerName || "-"}
                                        </div>
                                    </div>
                                    <div>
                                        <div className="text-sm text-gray-500">Status</div>
                                        <div className="mt-1">
                                            <span className={statusBadge(brand.status)}>
                                                {brand.status}
                                            </span>
                                        </div>
                                    </div>
                                        ...
                                        <div className="mt-1 text-sm font-semibold text-gray-800">
                                            {brand.addedByName ? (
                                                <span
                                                    ...
                                                    onClick={() => setSelectedUserPanelId(brand.addedBy)}
                                                >
                                                    {brand.addedByName}
                                                </span>
                                            ) : (
                                    ...
                                    <div>
                                        ...
                                        <div className="mt-1 text-sm font-semibold text-gray-800">
                                            {brand.updatedByName ? (
                                                <span
                                                    className="text-blue-600 cursor-pointer hover:underline"
                                                    onClick={() => setSelectedUserPanelId(brand.updatedBy)}
                                                >
                                                    {brand.updatedByName}
                                                </span>
                                            ) : (
