...
export default function ManufacturerList() {
    ...
    const openAdd = () => {
        setEditId(null);
        setFormContext("manufacturer-add");
        setFormPanelOpen(true);
    };

    const openEdit = (id) => {
        setEditId(id);
        setFormContext("manufacturer-update");
        setFormPanelOpen(true);
    ...
    return (
        <div className="fixed inset-0 flex flex-col bg-[#f5f6fa] overflow-hidden">
            <Header page="manufacturers" onSearch={handleSearch} />
            ...
            <div className="flex-1 w-full px-4 sm:px-6 lg:px-8 py-4 flex flex-col min-h-0 overflow-hidden">
                ...
                <nav className="mb-4 flex items-center space-x-2 text-sm font-medium text-gray-500">
                    ...
                    <span className="text-gray-800">Manufacturers</span>
                </nav>

                <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
                    {loading && (
                        <div className="bg-white rounded-xl border border-gray-200 p-8 flex items-center justify-center">
                            <Loader label="Loading manufacturers..." />
                        </div>
                    )}
                    ...
                    {!loading && !error && (
                        <DataTable
                            ...
                            columns={getManufacturerColumns(
                                (id) => setViewId(id),
                                openEdit
                            )}
                            data={manufacturers}
